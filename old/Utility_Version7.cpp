...
int main(int argc, char *argv[]) // argc -> argument count and argv -> argument values
{
 	if(argc == 1)
 	{
	 		printf("\nFormat : %s [-c file1 file2 ADD/CON][-y file1][-r file1][-b file1][-h file1 byte_count][-s filename string]", argv[0]);
			printf("\n\nOptions : ");
...
void compare_a(char f1[], char f2[])  // Compare the records of the file based on the addresses and the contents present in those addresses
{
	...
	while(1) // read and store all records present in file 1
 	 {
	  		c = fgetc(file1);// get the ':' character
			record[i].size = hex(fgetc(file1))*16 + hex(fgetc(file1)); // Record size(byte count) is denoted by 2 Hex Digits
			// record[] struct array stores records present in file1
			c1 = hex(fgetc(file1));  // Address field is denoted by 4 Hex Digits
			c2 = hex(fgetc(file1));
		  	c3 = hex(fgetc(file1));
		   	c4 = hex(fgetc(file1));  			  
	  		 
 	        record[i].address = c1 * 16 * 16 * 16 + c2 * 16 * 16  + c3 * 16 + c4;  // Hex to Decimal   
	  		record[i].add1 = c1*16 + c2;	// used for checksum
	  		record[i].add2 = c3*16 + c4;	// used for checksum
	  		record[i].record_type = hex(fgetc(file1))*16 + hex(fgetc(file1));	// record type is denoted by 2 hex digits
			
				for(j=0;j<record[i].size;++j)
					{
					 		record[i].data[j] = hex(fgetc(file1)) * 16 + hex(fgetc(file1));			   	 
				    }
			record[i].check_sum = hex(fgetc(file1))*16 + hex(fgetc(file1));	// check sum is denoted by 2 hex digits
			Line_Feed = fgetc(file1);	 // Line is present at the end of each record	   	    
			
			  i++;  
			  if(Line_Feed==0xFFFFFFFF) // Represents the end of file
			   		 break;
   }
	int rec_file1 = i; // stores number of records in file1
	i=0;
	fclose(file1);
    FILE *file2 = fopen(f2,"r");
    while(1) // read and store all the records present in file 2
 	 {
			...
			// new_record[] struct array stores records present in file2
   ...
   int k=0, l=0;
   for(i=0;i<rec_file1;++i)	// to combine the data from multiple records which have the same address
   {
   						 j=i+1;
						 ...
						 int f = l;							
   						 while(j<rec_file1)
   						 {
						   				 if(record[i].address == record[j].address)
						   				 {
												...
												record[j].address = -1; // assign the -1 to the record address which has been read , so it is not read again.
						  				  }
								j++;						  					  
			   				}
   ...
   l=0;
      ...
      for(i=0;i<rec_file2;++i) // to combine the data from multiple records which have the same address
	  {
   						   j=i+1;
						   if(new_record[i].address == -1) // address values is -1 only if it has been already read.
   						   continue;
   ...
   for(i=0;i<size_one;++i) // comparing the contents based on the addresses 
   {
						  						...
						  						if(one[i].address == two[j].address)
						  						{
												 	if(one[i].size == two[j].size)
													 {
													  			  for(m = 0;m< one[i].size;++m)
																	{
																		 ...
																		 {
																		  	 flag = -1; // if the data at byte m of the same address in the 2 files is different 
																		  	 break;
																		  }	
																	} 
																	break;
													   ...
													   {
									   	   			   	   flag = -3; // if the byte count the same address in the 2 files are different
									   	   			   	   break;
													    }			  
												 }
...
{// search and check if the given string s is present in the file 
	...
	for(i=0;i<j1;++i)
	{
		printf("\ncomp_data[%i] = %c ",i,(char)comp_data[i]);
	...
	printf("\nThe string exists in the file at %i byte", i);
}
