...
{
        if (argc == 1) {
                printf("\nFormat : %s [-c file1 file2 ][-y file1][-r file1][-b file1][-h "
                        "file1 byte_count][-v file1 max_byte_count][-s filename string]",
                        argv[0]);
                printf("\n\nOptions : ");
                printf("\n\n[-c] \t - \t Compare 2 Intel Hex Binary files based on "
                        "contents(data) present in the file. \n");
                printf("\n\n[-y] \t - \t Calculate the Cyclic Redundancy Check of one Intel "
                        "Hex Binary file. \n");
                printf("\n\n[-r] \t - \t Verify whether a file is in Intel Hex Binary File "
                        "format.  \n");
                printf("\n\n[-b] \t - \t Convert the given Intel Hex file to binary format  "
                        "\n");
                printf("\n\n[-h] \t - \t Convert the given binary file to Intel Hex file "
                        "format(byte count given)  \n");
                printf("\n\n[-v] \t - \t Convert the given Intel Hex file to another Intel "
                        "Hex file with given max byte size limit  \n");
                printf("\n\n[-s] \t - \t Check if a given string is present in the contents "
                ...
                switch (get_c(argv[1])) {
                case 'c':
                        if (argc == 4) // Number of arguments must be 4 [name_of_exe -c file1
                        // file2] (4 arguments in total)
                        {
